...
def main(_argv):
    ...
    while True:
        ...
        if return_value:
            mat = np.float32([[-1,0,600],[0,1,0]])
            frame = cv2.warpAffine(frame,mat,(600,450))
        ...
        # คลาสที่อนุญาตแบบกำหนดเอง (บรรทัด uncomment ด้านล่างเพื่ออนุญาตให้ตรวจหาเฉพาะคนเท่านั้น)
        # Allow_classes = ['คน']

        # หากเปิดใช้งานการตั้งค่าสถานะครอบตัด ให้ครอบตัดการตรวจจับแต่ละรายการแล้วบันทึกเป็นรูปภาพใหม่
        if FLAGS.crop:
            crop_rate = 30 # จับภาพทุก ๆ เฟรม (เช่น ครอบตัดรูปภาพทุกๆ 150 เฟรม)
