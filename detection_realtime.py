...
flags.DEFINE_string('output', './detections/webcam_5.avi', 'path to output video')
...
flags.DEFINE_boolean('save_data', False, 'perform license plate recognition')
...
def main(_argv):
    ...
    # รับชื่อวิดีโอโดยใช้วิธีการแยก
    video_name = video_path.split('/')[-1]
    video_name = video_name.split('.')[0]
    if FLAGS.framework == 'tflite':
        interpreter = tf.lite.Interpreter(model_path='./checkpoints/yolov4-tiny-416')
    ...
    # โดยค่าเริ่มต้น VideoCapture จะคืนค่า float แทน int
    width = int(vid.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(vid.get(cv2.CAP_PROP_FRAME_HEIGHT))
    fps = int(vid.get(cv2.CAP_PROP_FPS))
    # codec = cv2.VideoWriter_fourcc(*FLAGS.output_format)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter(FLAGS.output,fourcc, 20.0, (640,480))         
    ...
    while True:
        ...
        # image_data = cv2.bitwise_not(frame)
        ...
        crop_rate = 150 # จับภาพทุก ๆ เฟรม (เช่น ครอบตัดรูปภาพทุกๆ 150 เฟรม)
